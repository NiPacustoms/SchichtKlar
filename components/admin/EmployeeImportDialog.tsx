...
const CSV_TEMPLATE_HEADER = 'Name,E-Mail,Telefon,Jobtitel,Rolle,Aktiv,Gruppe';
const CSV_TEMPLATE_ROW = 'Max Mustermann,max@example.com,030 123456,MFA,nurse,Ja,SG1';
...
export function EmployeeImportDialog({ open, onClose, onSuccess }: EmployeeImportDialogProps) {
  ...
  const handleDownloadTemplate = () => {
    ...
    a.href = url;
    a.download = 'mitarbeiter_import_vorlage.csv';
    a.click();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if ((data.created ?? 0) > 0) {
        toast.success(`${data.created} Mitarbeiter angelegt`);
        onSuccess?.();
      }
    } catch (_e) {
    ...
    <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent>
        <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
          CSV mit Spalten: Name, E-Mail, Telefon, Jobtitel, Rolle, Aktiv, Gruppe. Erste Zeile kann
          die Kopfzeile sein.
        </Typography>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          {result && (
            <>
              <Alert
                severity={
                  result.errors.length > 0 ? (result.created > 0 ? 'warning' : 'error') : 'success'
                }
              >
                {result.created} Mitarbeiter angelegt.
              ...
              {result.errors.length > 0 && (
                <List dense sx={{ maxHeight: 200, overflow: 'auto' }}>
                  {result.errors.slice(0, 20).map((err, i) => (
                    <ListItem key={i}>
                      <ListItemText
                        primary={`Zeile ${err.row}: ${err.message}`}
                  ...
                  {result.errors.length > 20 && (
                    <ListItem>
                      <ListItemText
                        primary={`… und ${result.errors.length - 20} weitere Fehler`}
                        primaryTypographyProps={{ variant: 'body2' }}
                      />
                    </ListItem>
                  )}
