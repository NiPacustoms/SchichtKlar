...
export const shiftService = {
  ...
  async getById(id: string): Promise<Shift | null> {
    ...
    // WICHTIG: Prüfe auf window nur für client-seitige Checks, nicht für SSR-Blockierung
    // Während SSR wird Firebase nicht initialisiert sein, daher ist die db-Prüfung ausreichend
    try {
      ...
      return {
        ...
        companyId: (data as unknown as Partial<Shift> & { companyId?: string }).companyId,
  ...
  // Subscribe to shifts with filters (realtime)
  subscribeAll(
    ...
    try {
      let q = query(collection(getDb(), COLLECTION_NAME), orderBy('date', 'desc'));
      ...
      const unsubscribe = onSnapshot(
        q,
        (snapshot) => {
          const shifts: Shift[] = snapshot.docs.map(d => {
  ...
  async getByFacility(facilityId: string): Promise<Shift[]> {
    const db = getDb();
    if (!db) {
  ...
  async getByDateRange(startDate: Date, endDate: Date, companyId?: string): Promise<Shift[]> {
    try {
      ...
      const q = query(
        collection(getDb(), COLLECTION_NAME),
        where('companyId', '==', resolvedCompanyId), // WICHTIG: Filter nach companyId
  ...
  // Create shift
  async create(data: Omit<Shift, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> {
    try {
      // Hole companyId aus der Facility
      let companyId: string | undefined = (data as unknown as Partial<Shift> & { companyId?: string }).companyId;
  ...
  // Update shift status
  async updateStatus(id: string, status: Shift['status']): Promise<void> {
    try {
      const shiftRef = doc(getDb(), COLLECTION_NAME, id);
      await updateDoc(shiftRef, {
        status,
