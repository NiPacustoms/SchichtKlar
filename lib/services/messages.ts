...
export const messageService = {
  ...
  async updateChannel(id: string, data: Partial<Channel>): Promise<void> {
    ...
    try {
      const channelRef = doc(getDb(), CHANNELS_COLLECTION, id);
  ...
  async addParticipantToChannel(channelId: string, userId: string): Promise<void> {
    if (!db) {
      throw new Error('Database not initialized');
    }
    try {
      const channelRef = doc(getDb(), CHANNELS_COLLECTION, channelId);
  ...
  async removeParticipantFromChannel(channelId: string, userId: string): Promise<void> {
    if (!db) {
      throw new Error('Database not initialized');
    }
    try {
      ...
      if (channelDoc.exists()) {
        const data = channelDoc.data();
        const participants = data.participants || [];
        const filteredParticipants = participants.filter((id: string) => id !== userId);
